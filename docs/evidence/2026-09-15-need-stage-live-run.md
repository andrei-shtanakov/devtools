...
Итого семь кругов S6, три зоны major по очереди: сайты/лок (1–2), closure
(3–6), open calls (2, 4, 7). Решение — владельцу.

## Итог (решение владельца 2026-09-16 — остановка и передача)
...
| номера bundle-/tasks-/approval-PR, approved tasks-спека | **bundle-PR spec-runner#522**; tasks-/approval-PR — **нет**: прогон остановлен на S6 (`stopped_review`) после семи кругов терминального ревью бандла |
...
четыре узла ниже, авторинг одним проходом) семь кругов подряд получало
1–2 major от терминального ревью, зоны major сменяли друг друга (сайты и
lock → таксономия closure → процедура open calls). Каждый фикс — субагент
на PR-ветке, гейт `--candidate` зелёный, цепочка `upstream_hashes`
сверена; коммиты `a90bcf2`, `59a45a8`, `bef9887`, `95ebf36`, `57fe973`,
`3907996`. Открытые находки круга 7 — в вердикте ai-prosto на #522.

Что остаётся владельцу: (1) судьба #522 — доработать по кругу 7 (два
локальных major: порядок записи маркера `continuation_index` и сайт
процедуры open calls для `retry`/`watch`) и продолжить прогон
(`REVIEW_MAX_DIFF_BYTES=800000 make behaviour-run ARGS='resume --run-id
durable-continuation-checkpoint-evidence-20260915-4c2a3e'`), либо
...
Фикс круга 7 — коммит `2fec3f8` (решения шире буквы двух находок: `start()`
...
запушен. **request-changes** — 1 major high confidence (правило «любой
более поздний прогон workstream-а без open calls → отказ `needs-human`»
из круга 4 делает документированный путь восстановления недостижимым:
сама дверь `evidence close-call`, закрывающая open call, создаёт более
поздний прогон без open calls и тем самым навсегда блокирует restore —
BEH-09/AC-08 при этом утверждают, что после close-call restore
применяется; внутреннее противоречие трёх узлов), 1 major medium
confidence (`restore` пишет свой run-start в индекс до собственных
проверок — попадает под своё же правило отказа), 2 minor medium
(`doctor` в эфемерном scratch-проекте; перенос гварда не называет тип
отказа). Круги 1–3 (сайты/lock) закрыты; круг 8 — снова зона open calls
(Q-12/restore, начатая кругами 2/4/7). Решение о продолжении — владельцу.
