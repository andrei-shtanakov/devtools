...
### Правки владельца и ручной мерж #522 (2026-09-17, после круга 8 — БЕЗ подтверждающего круга ревью)

Круг 8 дал четыре находки: 1 major high confidence (правило круга 4
блокирует restore через собственную дверь `evidence close-call`), 1 major
medium confidence (`restore` пишет свой run-start в индекс до собственных
проверок — попадает под своё же правило), 2 minor (`doctor` в эфемерном
scratch-проекте; перенос гварда не называет тип отказа).

Владелец сам отработал ТОЛЬКО находку правила круга 4 (коммиты
...
GitHub (`973081b`, `mergedBy: andrei-shtanakov`) — минуя S7 раннера и
минуя терминальное ревью этих двух коммитов. Три находки круга 8 —
major medium confidence про `restore` и оба minor — остались
неотработанными; полный текст — в вердикте ai-prosto на spec-runner#522
(голова `2fec3f8` — это и есть круг 8; ранее здесь ошибочно стояла
`95ebf36`, голова круга 5). Это НЕ дополнительный круг терминального
ревью — счёт кругов S6 остаётся восемь (§ выше). Судьба этих трёх —
раздел «Остатки круга 8: доработка 2026-09-18» ниже.
...
друга: сайты и lock → таксономия closure → процедура open calls); семь
из них закрыты фиксами субагентов на PR-ветке, каждый раз с зелёным
гейтом `--candidate` и сверенной цепочкой `upstream_hashes` (коммиты
`a90bcf2`, `59a45a8`, `bef9887`, `95ebf36`, `57fe973`, `3907996`,
`2fec3f8`). Круг 8 закрыт частично: владелец коммитами `4104711`/`d6f11c5`
отработал только 1 major high confidence из четырёх находок и смержил
PR вручную, минуя S7 раннера и терминальное ревью этих двух коммитов —
major medium confidence про `restore` и 2 minor остались неотработанными
(вердикт ai-prosto на #522, голова `2fec3f8`); разобраны 2026-09-18 — две
из трёх закрыты (#524/#526), третья (`doctor`) не закрыта, раздел ниже. Четыре дефекта
devtools, найденные живым прогоном, исправлены и влиты: #248, #249 —
agent-merge; #250 — харнесс, мерж владельца; #253 (reconciliation мержа
вне S7) — agent-merge, 4 круга ревью.

По букве §9 спеки приёмка E2 НЕ завершена: `waiting_human_merge` раннером
не пройден (PR смержен человеком в обход S7), tasks-/approval-PR не
заведены, approved tasks-спеки нет. Что доказано живым прогоном: стадия
Need работает от `start` до published-брифа и materialize без ручных
вмешательств; disp-авторинг сходится; S3–S6 (с найденными и починенными
дефектами #248/#249/#250) и S7/S8-реконсиляция (после #253) доводят run
до `completed`. Чекбокс `spec-loop-need-stage` **остаётся открытым** —
решение о его закрытии (в т.ч. решение принять §9 частично выполненной) —
за владельцем, как и было зафиксировано ранее в этом документе. Три
находки круга 8, оставшиеся неотработанными на момент мержа #522,
разобраны 2026-09-18: две закрыты (#524/#526), третья (`doctor`) **не
закрыта** и вынесена в spec-runner#525 — раздел ниже. Следующий
шаг вне зависимости от этого решения — `make behaviour-tasks ARGS='--run-id
...
## Остатки круга 8: доработка 2026-09-18

Три находки круга 8 (вердикт ai-prosto на spec-runner#522, голова
`2fec3f8`), не отработанные до ручного мержа бандла, были проверены на
влитом дереве `ae0ed9e` — все три оказались живыми. Диффы владельца
(`4104711` + `d6f11c5`, 3 и 3 файла) касались ровно одной находки из
четырёх.

| Находка круга 8 | Итог |
|---|---|
| [major, medium] `restore` пишет собственный run-start в индекс до своих проверок и попадает под собственное правило отказа | закрыта: spec-runner#524 (`312d7f0`, мерж владельца) + #526 (`ef8b32e`, agent-merge) |
| [minor] перечень «цены» переноса autouse-гварда на `paid_call._spawn` не называет тип отказа | закрыта: #524 |
| [minor] `doctor` объявлен покрытым seam-ом «без правок», хотя платный вызов идёт в эфемерном scratch-проекте | **не закрыта**, вынесена в spec-runner#525 |

### Что вскрылось сверх круга 8

Ревью доработки нашло два дефекта, которых круг 8 не касался, и оба в
`master` на момент начала работы:

- **FR-05 содержал ложное утверждение в Must.** «Прогон того же
  workstream-а, начатый позже восстанавливаемого, — тоже отказ» — правило,
  которое реализация обязана нарушить, иначе закрыть open call ради
  восстановления и затем восстановиться нельзя ни одним путём. Переписано
  в #526.
- **Критерий шага 5 был невычислим.** Проверка (6) читает индекс
  workstream-а и `run-start.json`; поля «прогон добавил continuation-state»
  там нет, а косвенные признаки инвертированы в обе стороны (у
  неблокирующей двери `evidence close-call` checkpoint есть, у блокирующего
  `plan` — нет). Ключом сделан закрытый перечень подкоманд плюс acknowledged
  checkpoint, с механическим гвардом полноты
  `set(PAYING_SUBCOMMANDS) == BLOCKING | NON_BLOCKING`.

### Принятые пробелы

- **spec-runner#525** — сайт `doctor`: провенанс `doctor` не производится
  ни одним сайтом, `task_id` пробы сталкивается с реальной `TASK-001`
  проекта, checkpoint-ы пробы уходят в store вызывающего (и потому проба
  может стать кандидатом в «выход» отказа `restore`), относительный
  store-root уезжает со scratch-каталогом. Требует названного механизма и
  правки карты модулей — за пределами правки формулировок.
- **spec-runner#527** — окно доставки checkpoint-а. Решение владельца
  2026-09-18: fail-closed по неизвестности невозможен (он делает
  недостижимым путь «дверь `close-call` → restore»), поэтому прогон,
  убитый между записью mutation и доставкой, не блокирует, и его правка
  теряется молча. Закрывается сужением окна — синхронным ack
  mutation-checkpoint-а для подкоманд без платного вызова.

### Цена

Четырнадцать платных кругов терминального ревью: пять по #524, девять по
#526. Каждый круг находил дефект, который иначе уехал бы в реализацию;
ядро правила при этом правилось трижды (перечень → перечень + checkpoint →
единый предикат acknowledged), и каждая правка ядра открывала новую дыру в
периферии. Урок для конвейера — тот же по форме, что и урок про размер
бандла выше: правка нормативного текста в зоне, где нормы ссылаются друг на
друга (requirements ↔ design ↔ behaviour ↔ acceptance ↔ decomposition),
стоит кругов пропорционально числу мест, где норма пересказана; дешевле
там, где у вывода одно место, а остальное — ссылка.
