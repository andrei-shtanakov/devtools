...
**Что делает гейт** (`decomposition_guard`, fail-closed в обе стороны):

- `Mode: standard` в tasks допустим ТОЛЬКО для DT с машиночитаемым waiver'ом;
- waiver допустим только классу `characterisation` и только DT, все
  зависимости которого доставлены (`DONE`); иначе объявление отвергается с
  именем DT и причиной;
- waiver у `type: verify` — отказ: у verify свой режим (`verify_first`), и
  снимать RED ему нечем и незачем.

**Граница.** Waiver отменяет требование честного RED **для одной названной
задачи** и не отменяет ничего другого: ни negative control (он условие
класса, а не исключение из него), ни покрытие, ни гейты приёмки. Санкция
адресна: она дана перечисленным DT и на остальные задачи графа не
распространяется.
