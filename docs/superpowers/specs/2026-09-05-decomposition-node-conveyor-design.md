...
**Задача.** Для implement-задачи RED снимает только `Mode: standard`
(`EXECUTION_MODES = ("standard", "tdd", "verify_first")`, `config.py:30`);
заранее объявляемого, гейт-уважаемого waiver'а в spec-runner **нет**:
`ExecutorState.record_waiver` харнессом не вызывается и аннотирует уже
наблюдённый исход задним числом, а JSON `tdd-waiver/v1` гейт не читает.
Значит единственный исполнимый рычаг — `standard`, и декомпозиция отвергала
его по единственной причине: «молчаливый обход TDD без durable-причины и
санкции». Проекция и есть способ сделать `standard` НЕ молчаливым.
...
**Граница — что режим `standard` снимает на той стороне.** Установлено по
коду spec-runner (чтение, не правка; `gates.py`, `config.py`, `hooks.py`):

- снимает ровно ДВА гейта: `tdd.red` (подтверждённый RED-чекпоинт) и
  `tdd.claims` (сверка claims). Оба регистрируются только под
  `tdd`/`verify_first`, под `standard` пропускаются с
  `SATISFIED/SKIPPED`;
- **оставляет** всё остальное, и оно от режима не зависит: `review`-гейт
  при `review_policy: required`, `post_done.run_tests`,
  `post_done.run_lint` с `lint_blocking`, `auto_commit`, `run_review`;
- **negative control гейта на той стороне НЕ имеет вовсе** — ни одного
  упоминания в `src/`. Значит условие 4 класса держится **только пунктом
  чек-листа** и проверяется человеком на ревью задачи. Это самое слабое
  место проекции, и оно названо здесь именно поэтому: полагать, что
  spec-runner поймает невыполненный negative control, — ошибка.
