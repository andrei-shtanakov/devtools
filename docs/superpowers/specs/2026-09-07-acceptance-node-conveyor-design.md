...
2. **Инварианты покрытия** (проверяются гейтом):
   ...
   - каждый AC трассируется только к существующим FR/NFR; `scenarios` —
     только к существующим BEH; id в ссылках и заголовках несут суффикс
     не более одной строчной буквы (`[a-z]?` — класс BEH-18a, PR #148):
     грамматика ссылок `FR-\d+[a-z]?`/`NFR-\d+[a-z]?`/`BEH-\d+[a-z]?`;
...
| переходный режим | бандл без 25-acceptance.md ⇒ тот же fail-closed RuntimeError с процедурой (доавторить либо `--legacy-bundle=5`); словарь `3|4` зашит в ШЕСТИ операторских текстах — обновить все (миноры кругов 2–3): текст RuntimeError `_check_bundle_composition`, `ValueError` в `_dag_for`, help `--legacy-bundle` CLI, строка `make help` в Makefile, тело PR `deliver_conform`, таблица README |
