...
Дата: 2026-09-22. Статус: draft, **spec converged** (локальное ревью Codex, 4 круга:
4 blocker, 6 major, 6 minor → 1 blocker, 7 major, 5 minor → 1 major, 5 minor
→ 1 nit; все отработаны). Ждёт вычитки владельца (§9). Эпик:
...
**Уточнение D1, требующее подтверждения владельца (S1).** Гейт 1 «после
requirements» механически состоит из **двух** человеческих актов: charter и
...
Один, для владельца: подтвердить уточнение D1 (§2) — гейт 1 механически
состоит из двух актов (charter, затем requirements), итого шесть актов на
бандл; либо решить о смене контракта пина (вне этой спеки). Остальное —
S1–S13 — решения спеки, ждут вычитки.
