...
Дата: 2026-09-22. Статус: **accepted** — spec и pair converged (локальный цикл Codex),
уточнение D3 подписано владельцем 2026-09-22 (§2). PR devtools#341. Эпик:
...
**Уточнение D3 — подписано владельцем 2026-09-22:** «подтверждённая смена
версии доверенной политики терминализирует заявку с отдельной причиной
`policy_changed`, сохраняя её историю и evidence. После этого разрешён
новый candidate с новой версией политики и повторной авторизацией. Сеть,
недоступность источника и несоответствие содержимого пину оставляют заявку
живой и блокируют продвижение». Довод, из которого выросло уточнение:
буквальное
...
| S6 | Смена версии при живой заявке: на фазе 2 — установленный факт о паре версий → заявка `invalidated` с причиной вида `policy_changed: закреплена X, актуальная Y` (машинный префикс `INVALIDATION_POLICY_CHANGED = "policy_changed"` — отличим от прочих причин; запись заявки, включая `policy`, `candidate_pr`, `head_sha`, сохраняется целиком — история и evidence не стираются). Дальше — новый candidate с новой версией и повторной авторизацией. Команды перепиновки живой заявки нет: она превращала бы акт под одной политикой в акт под другой |
...
| `FORBIDDEN(superseded)` | **`invalidate_request`** с причиной `policy_changed: закреплена X, актуальная Y` (S6) и `RuntimeError`, называющий переход: новый candidate над теми же узлами с новой версией и повторной авторизацией |
...
1. PR пары (спека + план) — devtools#341, проза, агентский мерж после
   scope-аттестации.
2. Bootstrap репозитория политики — отдельное разрешение владельца
   (Task 1 плана).
3. PR кода по плану — мержит человек (`human-merge.sh` и authority-root в дифе).
4. **Не позже** мержа кода — правка правила волта (prograph-vault#147 расширяется):
...
Нет. Уточнение D3 подписано 2026-09-22; S1–S9 — решения спеки, вычитаны
вместе с ней в PR #341.
