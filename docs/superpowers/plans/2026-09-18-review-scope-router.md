...
## Все задачи — одним PR от `master`

#259 смержен 2026-09-18 09:42 UTC (`c9e047e`), парсер `lr_*` в `master` —
разделение на два PR, которое требовалось при открытом #259, больше не нужно.
Порядок задач сохраняется: Task 1 идёт первой, потому что Task 2 и Task 3
опираются на её разбор.

Ветка одна: `feat/review-scope-router` от актуального `master`.
