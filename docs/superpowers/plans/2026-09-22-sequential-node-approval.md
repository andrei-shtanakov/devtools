...
**Spec:** `docs/superpowers/specs/2026-09-22-sequential-node-approval-design.md` (accepted; D1 подписано 2026-09-22 — шесть актов, таблица соответствия трём гейтам). **Pair converged** 2026-09-22 (локальный цикл Codex, 3 круга).

## Global Constraints

- Волны = уровни DAG: W1 charter, W2 requirements, W3 behaviour-spec, W4 design + acceptance, W5 decomposition (S1); `wave_of(node) = level(node) + 1`.
...
- **Типы:** `wave_key(base, wave)`, `state.wave` (int, номер волны, 1-based), `propose_from_source(state, ops, node_ids, source_sha)`, `LevelResult(records, verdict, exit_code)`, `publish_wave(state, ops, key, level_result) -> int` — сквозные.
