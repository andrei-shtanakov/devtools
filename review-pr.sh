...
# Граница доверия кита: пуста, пока диапазон не сужен. Умолчание кита —
# граница совпадает с базой, и передавать его явно значило бы утверждать
# разделение там, где его нет.
trusted_base_arg=""
...
if [ "$targeted" -eq 1 ]; then
    ...
    # Кит обязан уметь принимать границу доверия ОТДЕЛЬНО от диапазона
    # (steward#181). Без этого сужение читает generated-декларацию и
    # курируемый контекст из коммита ВЕТКИ PR: автору достаточно объявить
    # свои файлы `linguist-generated`, и фикс-коммиты уедут под маркер
    # опущения, а кит выйдет нулём — approve поверх доставленного
    # request-changes (блокер круга 2, devtools#281). Feature-detect тем же
    # греп-приёмом, что у `--fingerprint-only`; отставший кит — отказ, а не
    # «сузим и понадеемся».
    grep -q -- '--trusted-base' "$trusted_kit_dir/local.sh" \
        || die 2 "--targeted: кит репо '$repo' не знает --trusted-base," \
            "то есть не умеет держать границу доверия отдельно от" \
            "диапазона. Сужение на таком ките читало бы" \
            "generated-декларацию и курируемый контекст из ветки PR." \
            "Ре-вендорьте кит (steward#181) или прогоните полное ревью."
    review_base="$lr_head"
    trusted_base_arg="--trusted-base origin/$base_ref"
fi
...
barrier_done=0
# Барьер вынесен в функцию, потому что точек вызова стало ДВЕ, а
# определение обязано остаться одно. В адресном режиме он выполняется до
# ЛЮБОГО успешного выхода, включая ветку прозы (решение владельца
# 2026-09-21): иначе `--targeted` после approve проходил бы без
# `--budget-override`, что и случилось на круге 2 devtools#281. Для
# обычного прогона место прежнее — ПОСЛЕ fp-блока: отпечаток и
# наследование вердикта бесплатны и бюджетом не ограничены, а ветка прозы
# у него срабатывает, лишь когда прозаический ВЕСЬ PR, то есть платного
# круга там не бывает вовсе.
enforce_review_barrier() {
    [ "$barrier_done" -eq 0 ] || return 0
    barrier_done=1
    # Журнал читается ЗДЕСЬ, при вызове, а не при инициализации: счётчик —
    # состояние на момент барьера. (Вынося блок в функцию, я эту строку
    # потерял, и `budget_used` оставался нулём — барьер молча не срабатывал
    # ни разу; поймали шесть бюджетных тестов.)
    [ ! -f "$budget_ledger" ] \
...
if [ "$scope" = "prose" ] && [ "$include_prose" -eq 0 ]; then
    if [ "$targeted" -eq 1 ]; then
        # Барьер ДО успешного выхода этой ветки — решение владельца
        # 2026-09-21: она выходит нулём, и без него `--targeted` после
        # approve проходил бы без `--budget-override` (блокер круга 2).
        #
        # Вызов стоит ЗДЕСЬ, а не перед всем блоком (блокер круга 3):
        # выше по течению лежат отпечаток и `--use-verdict`, которые модель
        # не зовут и бюджетом не ограничены по контракту CLAUDE.md. Барьер
        # перед ними ломал штатный двухфазный флоу адресного recheck —
        # вердикт оплачен, а публикация упирается в код 6.
        #
        # Порядок внутри ветки тоже значим: барьер РАНЬШЕ отказа по прозе.
        # После approve причина отказа — stop rule (код 6), а не «нет кода»:
        # круга нет вовсе, и узнать это оператор должен первым.
        enforce_review_barrier
        # Аттестация утверждает «этот PR платного ревью не требует». Под
        # сужением прозой классифицирован лишь хвост после прошлого ревью,
        # а PR содержит код: то же тело утверждало бы про весь PR то, что
        # верно про его хвост, и читатель различить не может. Поэтому
        # отказ, а не переписанный текст: у отказа один смысл.
        die 2 "--targeted: в диапазоне после прошлого ревью ($review_base)" \
            "нет кода — ревьюировать нечего, а аттестовать весь PR по его" \
            "хвосту нельзя. PR остаётся человеку; полное ревью — без" \
            "--targeted."
    fi
...
if [ "$fp_supported" -eq 1 ]; then
    set +e
    # shellcheck disable=SC2086 — cap_args/include_prose_arg/trusted_base_arg
    # проверены: только флаги, цифры и ref. Расщепление по пробелу намеренное.
    fp_out=$(run_kit \
        --base "$review_base" $trusted_base_arg \
        --head "$review_ref" --fingerprint-only \
...
# shellcheck disable=SC2086 — trusted_base_arg строится здесь же и
# содержит ровно флаг и ref; расщепление по пробелу — намеренное.
[ -z "$trusted_base_arg" ] || set -- "$@" $trusted_base_arg
