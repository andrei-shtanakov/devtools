...
| `todo_context.py` | context-pack по ОДНОМУ пункту `TODO.md`: строка и её отступное тело, `goal`/`notes` эпика из `epics.toml`, соседи по `@blocked_by`, доки, названные пунктом или упоминающие его `@id`, правила репо и (по `--issues`) исходный inbox-issue. Каждый источник отчитывается `read`/`absent`/`not_queried`/`error`, и `execute_allowed` считается по ним, а не по непустому тексту. Парсинг и граф — пакет `plan-fields`, поэтому `uv` + Python 3.12; `make todo-context ARGS='--repo <r> --id <id>'` |
