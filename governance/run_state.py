...
class RunState:
    ...
    # под §I12. Старые run.json без поля читаются как legacy — это
    # умолчание ДЕСЕРИАЛИЗАЦИОННОЕ, и флип дефолта (S13, 2026-09-23)
    # его не трогает: дефолт сменился у СОЗДАНИЯ прогона (CLI кнопки
    # и раннера). Поменять его здесь значило бы задним числом
    # объявить волновыми давно завершённые прогоны, и восстановление
    # из фактов GitHub искало бы candidate-PR там, где был бандл-PR.
