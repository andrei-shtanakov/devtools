...
3. «объявленное upstream-ребро без пина» steward ловит warn'ом, этот адаптер
   не ловит вовсе: цикл идёт по `artifact.meta.upstream_hashes` (что
   ЗАПИНЕНО), а не по графу профиля (что ОБЯЗАНО быть запинено) — `check_stale`
   графа даже не принимает. Свежеавторенный бандл без единого пина проходит
   prospective-stale молча. Часть этой дыры на практике закрыта побочно:
   `bundle_state.candidate_state` теперь метит узел без присутствующего
   upstream как `"blocked"` (см. `bundle_state.py`), а не молчит — но это
   не покрытие каждого непинованного ребра, только случай отсутствующего
   upstream-артефакта. Полное закрытие — candidate-контракт steward,
   который эту дыру устраняет by construction, не патчем адаптера.
"""
