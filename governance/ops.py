...
#: бюджет платных прогонов исчерпан либо stop rule (последнее ревью —
#: approve). Канон — контракт кодов в шапке `review-pr.sh` (devtools#258).
REVIEW_BUDGET_EXIT = 6
...
#: Совет «повторите обычный запуск» здесь намеренно отсутствует — журнал
#: бюджета ключуется по `slug#pr`, поэтому новая голова круга не открывает
#: и повтор без override отказал бы снова. Контур override не передаёт:
#: барьер существует, чтобы перерасход был решением владельца с названной
#: причиной (решение владельца 2026-09-20, devtools#258).
REVIEW_BUDGET_STOP = (
    "Бюджет исчерпан. Требуется решение владельца. Для осознанного "
    "продолжения запустите вручную с --budget-override \"<причина>\"."
)
