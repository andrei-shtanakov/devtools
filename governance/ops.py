...
_AUTHOR_DSL = {
    ...
    "decomposition": (
        ...
        "REQUIRED for type: verify (non-empty; declares the OBSERVATION "
        "group of files this verify DT watches) and FORBIDDEN for type: "
        "implement. checked_by (carried by scenarios) remains the single "
        "source of EDITING ownership — verifies is observation only and "
        "does not need to overlap the task's own scenarios/checked_by "
        "targets; files listed there are exempt from the single-owner "
        "invariant for THIS task only. Every BEH-* of the bundle MUST be "
        "covered by exactly one DT (no gaps, no duplicates). delivered_by "
