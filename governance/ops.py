...
_AUTHOR_DSL = {
    ...
    "decomposition": (
        ...
        "remains the single source of EDITING ownership. Single-owner "
        "exemption is narrow: a file is exempt from the single-owner "
        "invariant ONLY if it is listed in THIS task's own verifies AND is "
        "NOT also this same task's own checked_by target — list ONLY "
        "files this DT does not itself edit (owned by other DTs); a file "
        "this DT also covers via its own checked_by is NEVER exempt "
        "(ownership always beats observation). FATAL graph invariant "
        "(same rule as delivered_by, not a form recommendation): every "
        "file in verifies MUST be owned (via checked_by) by a DT that is "
        "in the transitive closure of THIS task's OWN depends_on — direct "
        "or transitive; if the owning DT is not reachable through "
        "depends_on, add an edge to it (or to a DT that transitively "
        "reaches it) — an owner outside the closure is a graph error, not "
        "a warning, and stops delivery. Every BEH-* of the bundle "
