...
_AUTHOR_DSL = {
    ...
    "decomposition": (
        ...
        "`negative_control: <selector>` is REQUIRED on every DT that "
        "declares `tdd_waiver` and FORBIDDEN on every other DT: it names "
        "the ONE test node id (`file::test`, one token, no spaces) that "
        "the implementer's NEGATIVE-CONTROL PATCH must turn red — the "
        "test lives in a file THIS DT owns through the checked_by of its "
        "own scenarios (a selector in another DT's file is rejected). "
        "spec-runner refuses a waived task that carries no control, so a "
        "waiver without it would deliver a task that never runs. You do "
        "NOT write the patch path: the bridge derives it from the task "
        "number (`spec/negative-controls/<ws-id>/TASK-NNN.patch`), and "
        "the implementer commits the patch there together with the work. "
