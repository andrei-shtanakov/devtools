...
def read_dag_state(
    ...
    доставки зовёт его же (с катовера PR #191 — зовёт, а не «будет
    звать»): у предиката два места применения и одна редакция —
    понадобится изменить, что считается честной одобренностью, менять
    надо здесь, и оба потребителя прочитают новое значение без единой
    правки у себя.
