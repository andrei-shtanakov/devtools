...
def _invalidate_downstream(
    ...
    удаление идёт best-effort без повторного branch-head lookup: этот путь
    сознательно не даёт гарантии подтверждённой очистки — сделай шаг
    блокирующим, и воркстрим, где удаление не подтверждается, не заведёт
    новую заявку никогда.
    """
