...
def _debt_notice(state: RunState, ops: Ops, legacy_bundle: int | None) -> str:
    ...
    Множество, которому это адресовано, не пустое и ровно одно:
    `approved`-узел с пустой либо стёртой подписью. Все прочие долги
    канонизация НЕ режет, они двигают `content_anchor`, и до no-op'а
    вызов не доходит вовсе (§I5, «какие долги §I5 способен спрятать»).
    Печатать поэтому будет редко — но именно в том случае, который иначе
    не виден ниоткуда.
