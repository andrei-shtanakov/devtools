...
def render_tasks_dt(
    ...
    for t in dt_tasks:
        ...
        if t.type == "verify":
            ...
            # Рычаг, снимающий RED у implement-задачи, в spec-runner
            # ровно один — `standard`; декомпозиция отвергала его только
            # за молчаливость, и проекция эту молчаливость снимает:
            # объявление durable лежит в DT, `Source:` задачи на него
            # указывает, а условия класса уходят пунктом чек-листа ниже.
            #
            # Ветка `elif`, а не второй `if`: waiver у verify-задачи гвард
            # не пропускает, но если бы пропустил — два режима спорили бы
            # за одну строку `**Mode:**`, и побеждал бы порядок строк
            # рендера. Пусть лучше побеждает тот режим, у которого он
            # собственный.
