...
def _debt_notice(state: RunState, ops: Ops, legacy_bundle: int | None) -> str:
    ...
    Множество, которому это адресовано, не пустое, и классов в нём **два**
    (правка по ревью #194; первая редакция говорила «один» и была
    неверна). §I5 слеп ровно к тому, что канонизация вырезает ЛИБО
    подменяет:

    - `approved`-узел с пустой либо стёртой подписью — `approved_by`
      и `approved_at` вырезаются;
    - `approved`-узел с разошедшимися `upstream_hashes` — записанный пин
      в канонические байты не попадает вовсе, он ПОДМЕНЯЕТСЯ каноническим
      блобом upstream'а (`_canonical_dag_hash`). Гейт же сверяет пин с
      СЫРЫМ блобом (`node_debt`) и долг видит.

    Классы сцеплены: стирание подписи у нетерминального узла меняет его
    сырой блоб, а значит разводит пин у каждого, кто его пинует, — один
    вход обычно даёт оба долга сразу.

    Все прочие долги канонизация берёт из файла как есть, они двигают
    `content_anchor`, и до no-op'а вызов не доходит вовсе (§I5, «какие
    долги §I5 способен спрятать»). Печатать поэтому будет редко — но
    именно в тех случаях, которые иначе не видны ниоткуда.
