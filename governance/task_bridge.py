...
#: КОНТРАКТНАЯ форма id результата (§3b.3), которую сверка ищет в уже
#: отрендеренном тексте задачи. Употребляется ровно для одного вопроса —
#: «нет ли в чек-листе результата, которого DT не объявлял»; на него без
#: формы не ответить, потому что незаявленный id неоткуда взять. Счёт
#: попаданий по ОБЪЯВЛЕННЫМ id её не использует (находка ревью PR #290):
#: там повтор формы был бы вторым её вычислителем рядом с гвардом.
#:
#: Читается РЕЗУЛЬТАТ рендера, а не промежуточные списки, из которых он
#: собран: проверка по ним была бы утверждением о себе и пропустила бы
...
def _assert_delivers_projected(
    ...
    # Пункты берутся В ЛЮБОМ состоянии отметки (находка ревью Copilot на
    # PR #290). Носитель обязательства — один и тот же пункт; перенос §I11
    # меняет в нём ровно символ отметки, и фильтр по `- [ ]` объявлял
    # отмеченный результат ПОТЕРЯННЫМ, отказывая в доставке. Конвейер
    # вставал бы на втором переиздании любой задачи, где исполнитель успел
    # отметить обязательство, — тем вернее, чем лучше шла работа.
    #
    # Форма пункта не переопределяется здесь: `_CHECKLIST_RE` —
    # единственное место, где она задана, и вторая её редакция разошлась
    # бы с первой молча.
    items = [line for line in block if _CHECKLIST_RE.match(line) is not None]
    for item_id in declared:
        # Попадания считаются по ОБЪЯВЛЕННОМУ id, а не по контрактной
        # форме (находка ревью PR #290). Форму судит гвард; повтори её
        # сверка здесь вторым вычислителем — на любом расхождении мост
        # сказал бы «результат не доехал» про пункт, который отрендерил
        # сам, то есть указал бы не туда, куда надо смотреть.
        hits = [
            line
            for line in items
    ...
    for d in task.delivers:
        ...
        own = [
            line
            for line in items
            if (m := _CHECKLIST_RE.match(line)) is not None
...
def _control_patch(ws_id: str, task_number: int) -> str:
    """Канонический путь патча негативного контроля задачи (devtools#336).

    Относительный, внутри репо-цели, рядом с tasks-спекой; один вычислитель
    на meta-строку и пункт чек-листа — два написания разошлись бы молча.
    """
...
def render_tasks_dt(
    ...
    for t in dt_tasks:
        ...
        if t.type == "verify":
        ...
        elif t.waiver is not None:
            # Проекция TDD-waiver'а (спека decomposition-node §3a).
            # `Mode: standard` выбирает ослабленный режим, но НЕ делает
            # задачу адресно waived сам по себе. Адресность несёт отдельный
            # `TDD-waiver` marker ниже; Source указывает на durable DT, а
            # условия класса доезжают человеку пунктом checklist.
            #
            # Ветка `elif`, а не второй `if`, защищает только единственность
            # строки `Mode`: waiver у verify-задачи гвард не пропускает,
            # но при регрессии два режима не должны спорить по порядку.
            # Checklist ниже отдельно ключуется по `t.waiver`; это не часть
            # защиты `elif`, а безопасное следствие того же guard-инварианта
            # (#198).
            lines.append("**Mode:** standard")
            # Машиночитаемый маркер — ОТДЕЛЬНОЙ строкой рядом, в форме,
            # которую разбирает `spec_runner.task.TDD_WAIVER`
            # (`\*\*TDD-waiver:\*\* (.+)`; разделитель ` · `, значения
            # без пробелов). Именно он, а не `Mode`, делает задачу адресно
            # waived на той стороне: предикат spec-runner —
            # «режим standard И маркер разобран», поэтому обычный
            # `standard` там остаётся нетронутым по построению.
            #
            # Проверки «маркер стоит только у waived-DT» здесь нет и не
            # нужно: мост — ЕДИНСТВЕННЫЙ писатель обеих строк, и они
            # печатаются из одной ветки одного `if`. Правило держится
            ...
            # платного вызова, терминально. Форма — потребителя:
            # `**Negative-control:** <путь> :: <селектор>`, разделитель
            # ` :: ` С ПРОБЕЛАМИ (внутри pytest-селектора `::` пробелов
            # не несёт и делением не задевается).
            #
            # Путь патча выводится ЗДЕСЬ, а не объявляется в DT: DT не
            # знает номера задачи, а мост — тот, кто задачу именует.
            # Подкаталог воркстрима — не украшение: TASK-ID уникален
            # только внутри одной tasks-спеки, а в репо их несколько
            # (у spec-runner — пять), и плоский
            # `spec/negative-controls/TASK-002.patch` двух воркстримов
            # перетирал бы друг друга. Селектор — из объявления DT
            # (`negative_control:`), гвард уже сверил его с checked_by-
            # файлами задачи; ничего здесь не угадывается.
            lines.append(
                f"**Negative-control:** {_control_patch(ws_id, idx)} "
                f":: {t.waiver.control_selector}"
            )
        if t.delivers:
        ...
        lines += ["", "**Checklist:**"]
        if t.waiver is not None:
            # Условия класса — ПУНКТОМ, а не прозой, и это прецедент
            # BEH-33 (DT-12): проза DT в промпт исполнителя не
            # подставляется вовсе, поэтому условие, оставленное прозой,
            # для исполнителя не существует. Пункт он обязан отметить.
            #
            # Пункт ОДИН на все условия: waiver принимается целиком либо
            # не принимается — отметить три условия из пяти и пойти
            # дальше контракт не разрешает, а пять отдельных пунктов
            # ровно это и позволяли бы.
            #
            ...
            # бы ПОБАЙТОВО одинаковыми. Перенос состояния §I11 требует
            # единственности текста пункта с обеих сторон
            # (`marked & _unique(...)`) и неуникальные выбрасывает —
            # значит при переиздании `[x]` терялась бы у всех троих,
            # притом что `DONE` самой задачи переносится. Получалась бы
            # DONE-задача с неотмеченным waiver-пунктом, а этот пункт —
            # единственный durable-след условия про negative control:
            # до spec-runner#428 гейта на него не было вовсе, а теперь
            # пункт ещё и называет путь патча и селектор (devtools#336).
            # Путь патча и селектор — В ПУНКТЕ, а не только в meta-строке
            # (devtools#336): промпт исполнителя у spec-runner строится из
            # чек-листа, meta-строку `**Negative-control:**` он не читает.
            # Без пункта исполнитель не узнал бы, куда положить патч и
            # что под ним обязано покраснеть, а контроль судится из
            # кандидат-коммита. Пункт по-прежнему ОДИН: контроль — это
            # условие 4 класса, названное конкретно, а не шестое условие.
            lines.append(
                f"- [ ] TDD-waiver {t.dt_id}, класс {t.waiver.node_class} "
                f"(санкция {t.waiver.sanction}; negative control — патч "
                f"{_control_patch(ws_id, idx)}, закоммиченный с работой, "
                f"под которым {t.waiver.control_selector} краснеет) — "
...
def _debt_notice(state: RunState, ops: Ops, legacy_bundle: int | None) -> str:
    ...
    день, когда переиздание понадобилось срочно. До сих пор требование не
    исполнялось — печаталась одна строка «апстрим не менялся».

    Множество, которому это адресовано, не пустое: ему адресованы
    **слепые зоны §I5**. Состав зон, имена классов и их сцепление
    определены в контракте, в блоке «Какие долги §I5 вообще способен
    спрятать», и здесь НЕ пересказываются — пересказ дрейфует ровно так
    же, как разошлись бы два вычислителя одного предиката, а этот же
    запрет мы держим для `read_dag_state`. Цена пересказа уже заплачена
    четырьмя неполными зачистками подряд (ревью #194).

    Перечня поэтому нет и здесь: функция печатает то, что вернул
    предикат, а не то, что о предикате помнит докстринг. Печатать будет
    редко — но именно в тех случаях, которые иначе не видны ниоткуда.

    **Порядок проверок это НЕ меняет, и подмены гейта здесь нет.** §I5 уже
    принял решение — вызов возвращает `noop` и RC 0 независимо от того,
    что скажет предикат; читается он ради ДИАГНОСТИКИ, после решения, а не
    до него. Отсюда два правила, и оба обязательны:

    - функция не отказывает НИКОГДА. Неустановленный факт (base не
      читается) даёт честную строку «долг установить не удалось», а не
      исключение: превратить бесследный no-op в RC 1 из-за диагностики
      значило бы гейтом его и сделать;
    - записей она не делает и волну не реконсилирует. `read_dag_state` —
      чистое чтение; `reconcile_wave_after_approved_dag` зовёт гейт, и
      только он.
    ...
    if not verdict.debts:
        return ""
    return "\n".join(
        ["в активном DAG есть долговые узлы (доставка их не пропустит):"]
        + [f"- {debt.render()}" for debt in verdict.debts]
    )


def _incomparable_reason(recorded: object, current: str) -> str:
    """Почему сверки §I5 не было — тем же правилом, что и сопоставимость.

    Выведено из `_comparable_anchors` ветка в ветку, иначе рядом с верным
    решением поселилось бы неверное объяснение. Случая ровно два, и оба
    дают один исход: величины нет вовсе либо она посчитана канонизацией
    другой эпохи; эпохи называются ОБЕ — как обе величины у разошедшегося
    пина.

    Чего текст не говорит НИ В ОДНОЙ ветке — «апстрим изменился»
    (требование контракта, решение владельца 2026-09-11). Этого никто не
    устанавливал: несопоставимость не есть различие, и подменять
    неустановленное установленным здесь запрещено тем же правилом, что и
    везде. Формулировка особенно соблазнительна именно тут — переиздание
    после `unavailable` действительно состоится, и «изменился» звучало бы
    правдоподобно, оставаясь ложным.
    """
    if not isinstance(recorded, str) or not recorded:
        return "предыдущая доставка не записала content_anchor"
    return (
        "её content_anchor посчитан канонизацией эпохи "
        f"{_canon_epoch(recorded)}, текущая — {_canon_epoch(current)}"
    )
...
def _approved_dag_or_refuse(
    ...
    потребителей предиката `read_dag_state` больше — к гейту добавилась
    диагностика no-op'а. Поэтому здесь и в `read_dag_state` перечислено
    поимённо, а не посчитано.
...
def deliver_superseded(
    ...
    if not comparable:
        # Объяснение печатается ЗДЕСЬ, а не только при записи ревизии
        # ниже, и это требование контракта, а не удобство: у легаси с
        # долгом гейт отказывает раньше записи, и без печати оператор
        # видел бы отказ §I12, не узнав, почему бесследного no-op'а не
        # случилось. «Изменился» текст не говорит ни в одной ветке —
        # никто этого не устанавливал; говорит он ровно то, что есть:
        # сверить не с чем.
        print(
    ...
    # будет, защищать нечего. Бесследным no-op'ом выше завершается ТОЛЬКО
    # воркстрим, чей долг попадает в слепые зоны §I5 — их состав живёт в
    # блоке контракта «Какие долги §I5 вообще способен спрятать», здесь
    # ссылка, не пересказ (devtools#195). Всё остальное доходит сюда — к
    # первой проверке переиздания, способной отказать.
    #
    # Легаси-воркстрим (запись v1) сюда доходит ВСЕГДА: сравнить v1 с v2
    # нечем, §I5 промолчал, и гейт — первая проверка, способная ответить.
    # RC 0 для такого воркстрима контракт больше не обещает (решение
    ...
    if not comparable:
        ...
        # Печати здесь НЕТ: то же объяснение уже прозвучало у сверки §I5
        # выше — там, откуда его слышно и при отказе гейта. Повтор в
        # одном вызове только создавал бы впечатление двух разных
        # событий.
