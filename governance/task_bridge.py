...
def deliver_superseded(
    ...
    вывод/сверка активного DAG предыдущей доставки (§I8) → provenance
    correction-PR (§I7) → проспективный anchor (§I2) → сверка с записанным
    anchor'ом предыдущей доставки → намерение (§I4, write-ahead) →
    доставка в НОВУЮ ветку `spec/<ws-id>-tasks-v<N>`.
