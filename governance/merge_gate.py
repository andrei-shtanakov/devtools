...
class Authority:
    ...
    Рулинг T3 (финальное ревью M-5): конструирование ``Authority`` из сырых
    строк конфига (репозиторного YAML, run-параметров) — обязанность
    вызывающей стороны (этап B); эта конструкция не валидирует значения на
    входе. Fail-closed держится в ``effective()`` ниже — allowlist, а не
    denylist: любое значение вне ``{"agent", None}`` на любой оси уже читается
    как human, включая мусор/опечатки/неизвестные строки. Так что
    недоверенный вход не обязан быть отфильтрован до конструктора — он
    fail-closed сам, только более грубо (все аномалии схлопываются в human,
    без разбора, что именно было не так).
    """
...
def decide(
    ...
    ``decide()`` ничего не знает о результате S4/candidate-среза (M-4): это не
    отступление от спеки §6 (сигнатура — ровно четыре оси), но красный
    candidate-гейт обязан быть отсечён runner'ом ДО вызова этой функции —
    `decide()` про право мержа, а не про качество бандла.
    """
