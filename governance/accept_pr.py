...
def _accept_on_head(
    ...
    if merge_code != 0:
        ...
        # Диагноз про базу — ТОЛЬКО на отказе форджи (код 4). Гвард
        # (код 3) уже назвал свою причину, и повтор в неё же и упрётся:
        # объявлять там «база уехала» значит увести оператора чинить не то.
        after = ops.pr_facts(repo_slug, pr) if merge_code == 4 else {}
