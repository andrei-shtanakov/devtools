...
#
# Разрешённый — как ПРАВИЛО, не как граница (devtools#184). Обвязка —
# обязательный поддерживаемый путь и defense-in-depth: агент, идущий
# штатно, упирается в её гварды, а нарушение правила видно в аудите
# (`merged_by`). Security boundary она не является и быть не может:
# сама она вызывает `PUT /repos/:slug/pulls/:n/merge`, и ровно тот же
# вызов доступен любому, у кого есть токен, — намеренный обход прямым
# API-вызовом здесь не предотвращается. Настоящая граница возможна только
# на стороне форджи (защита ветки, требующая статуса/ревьюера, которого
# агентская учётка выдать не может; запрет мержа для токена агента) — это
# внешняя конфигурация, не код обвязки.
#
...
#
# Категорические отказы — PR, который агент не вправе мержить НИКОГДА,
...
#   2. (снят правкой ADR-ECO-011 D5, 2026-09-14) ветка формы финализирующего
#      PR больше не отказ по форме: он подписи не создаёт, а лишь
#      записывает состоявшуюся; по умолчанию его мержит агент, а
#      human-политику несёт лейбл (гвард 3);
...
# здесь по имени, — ОТКАЗ (гвард 1), и это осознанная асимметрия:
...
#   2 — конфигурация, аргументы, профиль, состояние PR, неразобранные или
#       ещё не установленные факты (mergeStateStatus UNKNOWN) — повтор уместен;
#   3 — гвард: PR запрещён к агентскому мержу, остаётся человеку;
#   4 — форджа отклонила мерж (в т.ч. голова уехала после проверки);
#   5 — пин вызывающего разошёлся: вердикт относится не к этому состоянию.
#       Отделён от 3 намеренно: у 3 повтор не поможет (PR остаётся
#       человеку), у 5 — поможет, но только после нового вердикта. Один
#       код на оба означал бы, что вызывающий не может назвать процедуру,
#       не гадая (ревью #183, круг 8).
set -eu
...
# Финализирующая форма (`$finalize_glob`) по имени НЕ отказывает
# (ADR-ECO-011 D5): она удовлетворяет и глобу candidate, поэтому
# исключается ДО проверки candidate-формы, иначе гвард 1 назвал бы её
# актом одобрения.
...
# UNKNOWN/пусто/null — факт НЕ УСТАНОВЛЕН (форджа считает mergeability
# асинхронно, сразу после создания PR она его ещё не посчитала) — это код 2,
# как у любого неразобранного факта, и вызывающий вправе повторить позже
# (ревью #233: код 3 — гвард, у него повтор не поможет, смешивать нельзя).
case "$merge_state" in
    CLEAN|HAS_HOOKS|UNSTABLE|BLOCKED) : ;;
    UNKNOWN|""|null) die 2 "PR ${slug}#${pr}: mergeStateStatus='$merge_state' — \
факт не установлен (форджа ещё не посчитала либо ответ не разобран); \
неустановленный факт двери не открывает. Мерж не выполняется; повторите позже." ;;
    *) die 3 "PR ${slug}#${pr}: mergeStateStatus='$merge_state' не в числе \
разрешающих мерж (CLEAN|HAS_HOOKS|UNSTABLE|BLOCKED) — запрещающий факт \
двери не открывает. Мерж не выполняется." ;;
esac
