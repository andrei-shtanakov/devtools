...
- **Бюджет платных прогонов: 1 полный review + 1 адресный recheck**
  ...
  Адресный recheck (`--targeted`) **не доставлен** и в обвязке отсутствует:
  дизайн признан неверным 2026-09-20 (devtools#260, PR #281 в черновике).
  Сужение базы до прошлой отревьюированной головы переносит границу доверия
  кита внутрь PR — из `merge-base(--base, head)` кит берёт не только диапазон
  дифа, но и `.gitattributes`-декларацию `linguist-generated` и курируемый
  контекст `.github/codex/review-context.txt`; прошлая голова лежит на ветке
  PR, поэтому объявленный там `linguist-generated` прятал бы фикс-коммиты от
  модели. Чинится у источника: steward#181 разделяет диапазон дифа и
  `--trusted-base`. До этого второй круг — обычный полный прогон.
