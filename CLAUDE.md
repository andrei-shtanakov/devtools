...
- **Бюджет платных прогонов: 1 полный review + 1 адресный recheck**
  ...
  Адресный recheck — `review-pr.sh … --targeted` (devtools#260): база не
  ...
  полного ревью текущей головы он НЕ заменяет. Fail-closed отказы до
  вызова кита: нет маркера · **диапазон пуст по СОДЕРЖИМОМУ** (головы могут
  различаться при тождественном дереве — `--allow-empty`, коммит с его же
  revert; иначе кит вышел бы нулём и обвязка опубликовала approve поверх
  доставленного request-changes) · прошлая голова недоступна локально или
  не предок текущей (force-push/rebase) — у последних двух разные
  сообщения, случаи лечатся по-разному. Область ревью классифицируется по
  ТОМУ ЖЕ суженному диапазону: иначе фикс, тронувший только прозу, давал бы
  у обвязки `scope=code`, у кита — «всё отфильтровано» (код 5), а тот в
  fp-ветке вырождается в `die 3` «ревьюер не отработал». Барьер сильнее режима: после
  approve `--targeted` так же отбивается stop rule кодом 6.
