...
- **Бюджет платных прогонов: 1 полный review + 1 адресный recheck**
  ...
  посчитал `apply-threshold.sh`, и его решение пришло кодом выхода прошлого
  круга. Круг после approve `review-pr.sh` не открывает. Следствие, которое
  ...
  Адресный recheck — `--targeted`: база берётся из маркера последнего ревью
  ai-prosto, то есть проверяются изменения с прошлого круга, а не весь PR
  заново. Канон правила (SSOT): `../prograph-vault/authored/rules/git-workflow.md`,
  ...
  `review-pr.sh` отказывает (fail-closed); превышение — решение владельца с
  названной причиной (`--budget-override '<причина>'`), она уходит в журнал
