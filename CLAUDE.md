...
- **Мерж — агент по умолчанию** (ADR-ECO-011 «DarkFactory», 2026-08-30): при
  ...
  мержить человек (candidate-ветки §I12 — акт подписи; политика подписи читается из репозитория `andrei-shtanakov/approval-policy` по SHA, закреплённому в заявке (координаты — SSOT `contracts/approval-policy-source/v1/source.env` под authority-root; переменная `AUTHORIZED_APPROVER_ACCOUNTS` больше не читается, выставленная — отказ); независимость от исполнителя — в меру authority-root, граница форджи остаётся настоящей; лейбл `human-merge-required`; finalize-PR с ADR-ECO-011 D5 мержит агент по умолчанию — но у свежего finalize-PR нет одобряющего ревью, которого требует правило репозитория, поэтому `--approve-node` сперва отказывает с названным шагом: опубликовать scope-аттестацию `review-pr.sh <pr>` (prose-only — без вызова модели) и повторить вызов, после чего мержит тот же агент; проверка предварительная, окончательно решает форджа; **волновой режим — ЕДИНСТВЕННЫЙ** (S13, решение владельца 2026-09-23: прежний путь бандл-PR удалён из исполнения после двух живых приёмок). `--waves` принимается и ничего не меняет, `--legacy` принимается парсером и ОТКАЗЫВАЕТ с названной причиной, `resume` прогона прежнего режима отказывает бесследно (исключение — verification-потомок: он наследует режим родителя, несёт перенесённый факт мержа со ссылкой `source` и доигрывает ТОЛЬКО S8; допуск — полный предикат devtools#384 §2a, и WS-lock держит ЦИКЛ, а не каждую попытку — провальный брат лок не удерживает), восстановление леджера по бандл-PR отказывает вместо реконструкции — но историю это не трогает: `rs.load`, `status`, консоль и WS-lock читают прежние леджеры как прежде. Спека `docs/superpowers/specs/2026-09-22-sequential-node-approval-design.md`: узлы одобряются волнами по уровням DAG (W1 charter, W2 requirements, W3 behaviour-spec, W4 design+acceptance одной заявкой, W5 decomposition), заявка волны несёт байты ветки волны (`source_sha`), edge-check обязателен на каждой волне и публикуется в candidate evidence-коммитом и одобряющим ревью с маркером `ai-prosto-edge-check`, finalize мержит агент на resume после scope-аттестации, `--reopen <node>` переоткрывает узел и ведёт `stale` нижние уровни через переодобрение по base; человеческих актов на бандл — 6),
