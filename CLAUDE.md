...
| `attest-vendor.sh` | детерминированная аттестация целостности вендор-копии review-kit на волновом PR ре-вендора (решение владельца 2026-09-19) — публикует `--approve` от `ai-prosto` (тот же профиль, что у `review-pr.sh`), формально закрывая требование одобряющего ревью там, где содержательного ревью нет и не нужно: предмет — побайтовая копия файлов кита из апстрима. Материализация головы PR и профиль публикации — тем же приёмом, что у `review-pr.sh` (служебный ref + временный detached worktree, рабочее дерево целевого репо не трогается). Ядро — НЕЗАВИСИМАЯ побайтовая сверка каждого члена кита из `scripts/review/PIN` головы с тем же файлом в апстриме (`../steward`) на коммите, названном в строке `# SOURCE: steward @ <sha>` шапки PIN: зелёный `checksum.sh` доказывает лишь то, что копия соответствует своему же PIN, а не то, что PIN указывает на нужный апстрим — подменённый PIN вместе с подменёнными файлами прошёл бы `checksum.sh` зелёным, но не эту сверку. Только после чистой сверки с апстримом выполняется `scripts/review/checksum.sh --pin` в дереве головы. Маркер — собственный (`ai-prosto-vendor-attestation`), без подстроки `<!-- codex-terminal-review `, чтобы потребители протокола не прочли аттестацию как модельный вердикт; тело явно называет отказ от содержательного ревью (модель не вызывалась, семантика не проверялась). Коды выхода: 0 — чисто (или `--dry-run`), 2 — конфигурация/аргументы/состояние PR/публикация, 3 — сверка не прошла (расхождение с апстримом или красный `checksum.sh`), 4 — голова уехала между сверкой и публикацией |
...
- **Бюджет платных прогонов: 1 полный review + 1 адресный recheck**
  ...
  Адресный recheck (база — прошлая отревьюированная голова) каноном
  предусмотрен, но в обвязке пока не реализован: вынесен отдельно (devtools#260).
