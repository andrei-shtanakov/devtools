...
| `human-merge.sh` | триггер ЧЕЛОВЕЧЕСКОГО мержа по команде (ADR-ECO-011 D6): мерж от профиля оператора, `~/.config/review` — отказ; логин сверяется с политикой `approval-policy` по пину из тела candidate-PR. **Защиту ветки не обходит** — `--admin` нет и не будет (devtools#277, решение владельца 2026-09-23): под ruleset'ом с обязательным ревью мерж пройдёт только при наличии ревью на PR. В волновом режиме (дефолт с 2026-09-23) оно есть всегда — candidate несёт одобряющее ревью edge-check, и за две живые приёмки `--admin` не понадобился ни разу; в прежнем пути у candidate ревью нет, и там акт совершается либо после публикации ревью, либо руками оператора. «Единственным способом совершить акт» команда не называется — это было бы обещанием, которого контур не держит |
