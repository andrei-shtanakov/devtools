...
- **Проза модельному ревьюеру не отдаётся** (решение владельца 2026-09-18):
  ...
  **scope-аттестацию** от ai-prosto вместо вердикта: модель не вызывается,
  ...
  ту же scope-аттестацию (`publish_scope_attestation`, общая с ранней
  классификацией), бюджет не списывается — модель не звалась. Отличие от
  раннего пути: обвязка могла классифицировать PR как кодовый (например,
  репо-конфиг `review-scope.env` шире вшитого правила) — решает кит, его
  слово последнее. Оператору нужен полный прогон вопреки фильтру (ранней
  ...
  Repo-конфиг `review-scope.env` читается китом из base и только расширяет
  область ревью (может добавить пути под платное ревью, не может исключить).
