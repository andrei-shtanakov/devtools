...
help:
	...
	@echo "  make todo-work ARGS='--repo <r> --id <id> [--mode execute]' — прогон одного пункта; execute только при execute_allowed"
	...
	@echo "  make spec-loop SUBJECT='…' REPO=… ARGS='[--brief <path>]' — операторская кнопка: optional approved discovery source → start → мерж бандла (человек) → одобрение узлов (человек, --approve-node) → повтор → deliver tasks-спеки → approve (человек); E1 принимает готовый brief, но не запускает interview; merge-authority жёстко human"
...
todo-work:  ; @uv run --frozen python ./todo_worker.py --root $(WORKSPACE) --manifest $(MANIFEST) $(ARGS)
