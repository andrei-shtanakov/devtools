...
def test_tools_section_is_walked(drift) -> None:
    """Секция обходилась мимо: `comps` собирался только из cores и apps."""
    import inspect
    source = inspect.getsource(drift.main)
    assert 'manifest.get("tools"' in source, "секция [tools.*] не попадает в обход"
...
def test_a_pinned_tool_without_pyproject_is_info_not_warn(drift, tmp_path) -> None:
    ...
    _repo(tmp_path, "robin-toolkit")
    found = drift.check_component("robin-toolkit", _tool(), tmp_path)
...
def test_a_publishing_component_without_pyproject_still_warns(drift, tmp_path) -> None:
    ...
    found = drift.check_component(
        "core", _tool(publish="pypi", install="pypi"), tmp_path)
    vers = [x for x in found if x["kind"] == "no_pyproject_version"]
