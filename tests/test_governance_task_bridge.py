...
#: Блок контракта, который ОДИН определяет состав слепых зон §I5.
_BLIND_ZONES_BLOCK = (
    "**Какие долги §I5 вообще способен спрятать — ЕДИНСТВЕННОЕ МЕСТО.**"
)

#: Числительное, квантифицирующее классы/зоны/долги. Границы слова
#: обязательны: без них «двухфазным» читается как «двух».
_COUNT = (
    r"(?:единствен\w+|один|одна|одно|одного|два|две|двух|три|оба|обе)\b"
)
_ZONE_NOUN = r"(?:класс\w*|зон\w*|долг\w*)"
...
def test_blind_zones_composition_is_stated_in_exactly_one_place() -> None:
    ...
    Тест и есть тот критерий, которым это проверяется: «я прошёл и
    посмотрел» пропустило четыре захода подряд, причём дважды — места,
    правленные тем же патчем. Докстринги вдобавок не исполняются, и
    расхождение докстринга с телом теста зелёное навсегда; здесь оно
    краснеет.
    """
    import re
    ...
    for path in targets:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if pattern.search(window) and subject.search(near):
                offenders.append(f"{path.name}:{i}: {line.strip()[:120]}")
