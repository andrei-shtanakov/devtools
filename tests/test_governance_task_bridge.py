...
#: Слова, которые основа СЧЁТА ловит, а счётом они не являются. Список
#: ЯВНЫЙ, чтобы исключение было видно, а не растворялось в неполноте
#: регекспа: неполный регексп молча недосчитывает, явный список —
#: предъявляет, что именно прощено, и приглашает спорить. Сверяется с
#: группой счёта, а не со всем спаном (devtools#195, дыра 2): прощение по
#: промежуточному слову («одобренный») отпускало счёт рядом с именем зоны.
#: Прежние «одобр»/«должн» из перечня сняты — ни одну основу счёта они не
#: покрывают и держались только на сверке по всему спану.
_NOT_A_COUNT = ("двухфазн", "двига", "двиг", "движ")

#: То же для имени зоны: «долговечн» ловится основой `долг`, зоной не
#: является. Сверяется с группой зоны.
_NOT_A_ZONE = ("долгове",)
...
def _blind_zone_offenders(text: str, name: str) -> list[str]:
    ...
    # Именованные группы: прощение сверяется с ТЕМ, что прощается, — счёт
    # с основами счёта, зона с основами зоны, — а не со всем спаном.
    pattern = re.compile(
        rf"(?:(?P<c1>{_COUNT})\W+(?:\w+\W+){{0,2}}(?P<z1>{_ZONE_NOUN})"
        rf"|(?P<z2>{_ZONE_NOUN})\W+(?:\w+\W+){{0,2}}(?P<c2>{_COUNT}))",
        re.IGNORECASE,
    )
    ...
    for i, line in enumerate(lines, 1):
        ...
        # левое совпадение прятало нарушителя правее в той же строке.
        for hit in pattern.finditer(window):
            count = (hit.group("c1") or hit.group("c2") or "").lower()
            zone = (hit.group("z1") or hit.group("z2") or "").lower()
            if any(stem in count for stem in _NOT_A_COUNT):
                continue
            if any(stem in zone for stem in _NOT_A_ZONE):
                continue
...
def test_blind_zones_guard_judges_every_match_not_the_leftmost() -> None:
    """Дыра 1 (devtools#195): гвард судил по ПЕРВОМУ совпадению окна.

    Левое совпадение, прощённое основой из `_NOT_A_COUNT` («долговой узел
    двигает» — «двига»), пропускало всю строку, и пересказ со счётом,
    дописанный правее в том же окне, уходил в пропуск. Негативный тест
    прежней редакции этого не видел: он сажал нарушителя в конец файла, где
    прощаемой фразы перед ним нет.
    """
    spiked = (
...
def test_blind_zones_guard_forgives_the_count_not_the_whole_span() -> None:
    """Дыра 2 (devtools#195): основы прощения сверялись со ВСЕМ спаном.

    «доходит {} одобренный класс» прощалось основой «одобр» из слова между
    счётом и именем зоны — хотя счёт стоит рядом с зоной. Прощать можно
    только само числительное (группу счёта), не промежуточные слова.

    Фикстура берёт промежуточное слово с основой из ДЕЙСТВУЮЩЕГО перечня
...
def test_blind_zones_guard_still_forgives_true_non_counts() -> None:
    """Прощение осталось точечным: основа, которая счётом не является,
    прощается — в своей группе, а не за счёт соседей."""
    for text in (
        "Долговой узел двигает зону сравнения, см. §I5.",
        "Двухфазный класс доставки описан в §I5 без no-op'а.",
    ):
        assert not _blind_zone_offenders(text, "синтетика"), text
