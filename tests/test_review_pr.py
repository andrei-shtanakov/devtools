...
Контракт кодов выхода review-pr.sh:
  ...
  6 — барьер отказал (бюджет платных прогонов либо stop rule) — прогон
      возможен, но требует решения владельца (devtools#258). Пятёрка
      пропущена: это код КИТА «всё отфильтровано как проза».
"""
