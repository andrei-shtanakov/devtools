...
#
# ГРАНИЦА, которую команда не переходит (devtools#277, решение владельца
# 2026-09-23): защиту ветки она не обходит. Флага `--admin` у неё нет и не
# будет — выдать обвязке право обходить защиту значило бы отдать ей то, чем
# защита и является. Под ruleset'ом, требующим одобряющего ревью, мерж
# пройдёт ТОЛЬКО если ревью на PR есть. В волновом режиме (дефолт с
# 2026-09-23, S13) оно есть всегда: candidate несёт одобряющее ревью
# edge-check от ai-prosto, и за две живые приёмки `--admin` не понадобился
# ни разу. В прежнем пути (§I12 одним бандл-PR) у candidate одобряющего
# ревью нет — там команда упрётся в 405, и это НЕ дефект команды: акт
# совершается либо после публикации ревью, либо руками оператора в
# браузере. Называть здесь `make human-merge` единственным способом
# совершить акт было бы обещанием, которого контур не держит.
#
...
usage() {
    ...
    echo "  защиту ветки НЕ обходит (--admin нет): под ruleset'ом с обязательным" >&2
    echo "  ревью мерж пройдёт только при наличии ревью на PR (devtools#277)" >&2
}
