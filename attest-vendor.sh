...
# Дедуп НЕ здесь: он стоит ПОСЛЕ сверки (см. ниже, перед публикацией) и
# ключуется полным маркером, включая upstream. Ранний выход по одной лишь
# голове делал бы недостижимым то, что шапка и тело обещают оператору как
# средство от протухания: апстрим уехал, голова не двигалась, повторный
# прогон — и он бы вышел нулём, не выполнив ни одной проверки.
...
# Дедуп по ПОЛНОМУ маркеру (голова И апстрим): сверка уже выполнена и чиста,
# осталось решить, не публикуем ли мы то же самое второй раз. Ключ включает
# upstream именно потому, что при уехавшем апстриме прогон обязан был дойти
# сюда и либо покраснеть на сверке, либо опубликовать НОВУЮ аттестацию с
# новым upstream — а не выйти нулём по совпадению головы.
if ! dedup_seen=$(printf '%s' "$reviews_json" | jq -rs --arg m "$marker" \
    '([ .[][] | select(.user.login == "'"$REVIEW_LOGIN"'")
       | select((.body // "") | contains($m)) ] | length) > 0' \
    2>&1); then
    die 2 "не удалось проверить дедуп по прошлым ревью (jq): $dedup_seen"
fi
if [ "$dedup_seen" = "true" ]; then
    echo "ЗАМЕТКА: аттестация с этой головой и этим апстримом уже" \
        "опубликована — сверка повторно выполнена и чиста, публикация не" \
        "нужна."
    exit 0
fi
