...
- [x] Тег на строке продолжения не виден прибору: поднять PF-ID-MISSING, детектор оставить узким @owner:github:andrei-shtanakov @id:tag-placement-mid-line-blind-spot @epic:eco.plan-fields — PR этой ветки
      ...
      чужие `@id` постоянно.
- [ ] Валидация пинов `[tools.*]` в check-release-drift @owner:github:andrei-shtanakov @id:release-drift-tools-pins @epic:eco.plan-fields
      ...
      решение владельца.
      Поэтому пункт остаётся ОТКРЫТЫМ, хотя домашняя половина сделана (PR
      #139). Пометить `[x]` значило бы сказать соседу «доставлено»: у vault на
      этот `@id` встанет ребро, и закрытие пункта поднимет у них
      PF-BLOCKER-STALE — сигнал «ожидание доставлено, действуй», — при том что
      required-гейт зонтика секцию всё ещё не видит. Закрывать после обновления
      копии в зонтике (ревью PR #139, круг 2).
