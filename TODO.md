...
- [ ] Обвязка §I12 ждёт обязательные проверки перед мержем finalize; `human-merge.sh` без `--admin` под защитой ветки @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:i12-finalize-merge-vs-branch-protection
      ...
      **волновой candidate несёт одобряющее ревью edge-check** от `ai-prosto`.
      ...
      1 детерминирована и повторится 5 раз; без правки второй прогон измерит
      контур, которому на каждой волне нужна нянька оператора, — а
      сертифицировать флипом дефолта предполагается контур без неё.

- [ ] Диагностика волнового контура врёт в двух местах: пауза после отказа мержа finalize называет уже влитый candidate, `edge-findings.txt` переживает состояние, которое описывал @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:waves-diagnostics-name-wrong-state
      ...
      которое описывал. Критерий закрытия и регрессия — те же, что там:
      файл снимается на входе в шаг, а не только при записи новых находок.
