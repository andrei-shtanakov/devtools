...
- [ ] Ловить опору на удаляемую сущность: пункт не должен строиться на том, что удаляет другой пункт графа @owner:github:andrei-shtanakov @id:plan-check-deleted-dependency @epic:eco.plan-fields
      Пункт не должен опираться на сущность или путь, который удаляет другой
      пункт того же графа; проверка сопоставляет ТЕЛА пунктов, используя данные
      `todo_context`, без введения нового тега.
      ...
      Почему без нового тега: путь `scripts/harness/claude-review` уже назван
      в теле обоих пунктов, и требовать от автора третьего тега значило бы
      ...
      как самая проверяемая форма; (2) что считается «удаляет» — глагол в теле
      ненадёжен, надёжнее пересечение: пункт A называет путь P, пункт B
      называет тот же P и его строка/тело говорит об удалении; (3) порог шума:
      два пункта, упоминающих один путь, — норма (так выглядит любая
      совместная работа), находкой должна быть только пара «опирается» ×
      «удаляет»; (4) severity: это предупреждение автору плана, не гейт CI —
