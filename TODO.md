...
- [ ] Новые модули «Предмета» DT доезжают в чек-лист обязательством, а не теряются между предметом поставки и наблюдаемым поведением @owner:github:andrei-shtanakov @id:task-bridge-subject-deliverables-unpinned @epic:eco.dark-factory
      ...
      **Порядок поставки фиксирован** (решение владельца 2026-09-21):
      ...
      Следствие для гейта: `RunState.allow_legacy_dt` несёт ПЕРЕХОДНЫЙ
      дефолт `True` и обязан быть флипнут в `False` срезом 3, вместе с
      авторингом, пишущим `dt_contract_version: 2`. Строгий дефолт
      сегодня покрасил бы гейт на каждом бандле, который конвейер создал
      сам, — то есть поставил бы барьер раньше того, что он охраняет.
      Документ, объявивший версию 2, проверяется полностью независимо от
      флага; ужесточающая форма флага доступна оператору уже сейчас,
      послабляющая через `spec_loop` — не проброшена (петля берёт дефолт
      `start`, поэтому флип среза 3 дойдёт до неё сам). «Любое
