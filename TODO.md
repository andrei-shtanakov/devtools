...
- [ ] Вызывающий контур исполняет stop rule ревью механически, а не прозой: targeted recheck только после блокирующей находки, non-gate находки круга не открывают, адресный режим `--base <последняя отревьюированная head>` @owner:github:andrei-shtanakov @id:review-loop-limit-enforcement @epic:eco.tooling @blocked_by:steward#181
      ...
      **Адресный recheck (`--targeted`) НЕ доставлен: дизайн признан неверным
      2026-09-20** (devtools#260, PR #281 в черновике; ожидание держит
      тег на строке чекбокса выше). Он был вырезан из #259 после трёх кругов ревью, давших
