...
- [x] Тег на строке продолжения не виден прибору: поднять PF-ID-MISSING, детектор оставить узким @owner:github:andrei-shtanakov @id:tag-placement-mid-line-blind-spot @epic:eco.plan-fields — PR этой ветки
      ...
      чужие `@id` постоянно.
- [x] Валидация пинов `[tools.*]` в check-release-drift @owner:github:andrei-shtanakov @id:release-drift-tools-pins @epic:eco.plan-fields — PR этой ветки (домашняя копия; гейт зонтика — вторая половина, см. ниже)
