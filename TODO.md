...
- [x] Политика подписи §I12 приходит из источника, который проверяющий не может изменить @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:approver-policy-trusted-source — PR этой ветки (код по плану #341)
      ...
      **Решение владельца 2026-09-21: выбран доверенный источник,
      ...
      молчаливый пересчёт.
      **Решение владельца 2026-09-22 (место и контракт):** отдельный
      репозиторий политики без права записи у исполнителя; candidate
      закрепляет версию по commit SHA, finalize перечитывает из источника;
      недоступность/пустота/смена версии — отказ с сохранением заявки;
      ...
      до мержа. **Task 1 выполнен 2026-09-22 по разрешению владельца («да,
      создавай»):** `andrei-shtanakov/approval-policy` — публичный, соавторов
      нет, ruleset `main` (deletion, non_fast_forward, required_linear_history,
      pull_request без обязательных ревью, bypass пуст); первая версия политики —
      коммит `12d198fb4db9ed6fbb1c5b00af308eeb31e4ec22`, отпечаток `v1:31bf1658…`
      (тот же, что в прошлых ледгерах). Сверено живым чтением:
      `policy_snapshot(RealOps())` → FOUND; под профилем ai-prosto — тот же SHA.
      Код влит #344 мержем владельца. Ловушка стенда: SHA в стабах обязаны быть hex
...
- [x] Спека: последовательное одобрение узлов бандла — человеческий гейт переносится ВНУТРЬ авторинга, нижний узел пишется против уже одобренного и запиненного верхнего; три гейта (после `10-requirements`, после `15-behaviour-spec`, после пары `20-design`+`25-acceptance`), `30-decomposition` закрывается вместе с бандлом; `_step_authoring` перестаёт писать шесть узлов одним заходом, S5/S6/S7 повторяются на каждом гейте, паузы по идиому `waiting_human_merge` @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:sequential-node-approval — код PR #346 (Tasks 1–11 плана); живая приёмка — отдельным пунктом ниже
      ...
      **Решения владельца 2026-09-22:** три гейта (после requirements,
      после behaviour-spec, после пары design+acceptance); переоткрытие
      верхнего сохраняет нижние артефакты, делает их одобрения `stale` и
      блокирует продвижение, ничего не удаляется и не переписывается;
      переодобрение нижних — после проверки против новой версии верхнего;
...
- [x] Живая приёмка волнового режима (спека sequential-node-approval §6.3): `make spec-loop … ARGS='--brief … --waves'` на крошечном предмете; evidence `docs/evidence/<дата>-waves-live-run.md` — 6 человеческих актов, edge-check evidence и его ревью на каждом candidate, ноль платных ревью моделью на candidate/finalize, ноль правок узлов после одобрения без `--reopen`; после двух живых прогонов — дефолт `waves` и удаление прежнего пути (S13) @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:waves-live-acceptance
      S13 ЗАКРЫТ 2026-09-24: дефолт переключён (#375), прежний путь удалён из исполнения тремя поставками — #385 перенос покрытия на волны (96 тестов переведены, 1 сохранён для истории, 32 размечены под снос), #387 стражи отказа (resume, --legacy, verification-потомок) и снятие 14 недостижимых тестов, и эта поставка — пять шагов кортежа, `_step_merge`, `_LEGACY_STEPS`, 16 развилок `_waves`, вход через восстановление по бандл-PR закрыт отказом. Чтение истории не тронуто: 16 реальных леджеров прочитаны по копии (оригиналы сверены хэшами), фикстуры каждого класса сняты с реальных записей и закреплены для CI (`tests/governance_fixtures/historical_ledgers/`). Остаток — восстановление верификации devtools#384, вне объёма S13.
      ...
      **Прогон 1 из 2 выполнен 2026-09-22**, предмет — devtools#173 (реальный
      открытый дефект, не филлер); run-id
      `stray-md-disables-i8-for-legacy-v1-20260922-51772c`, статус `completed`,
      evidence — `docs/evidence/2026-09-22-waves-live-run.md`. Все четыре
      критерия §6.3 выполнены: 6 человеческих актов (candidate #348/#350/#352/
      #354/#356, все `merged_by=andrei-shtanakov`; конверты #349/#351/#353/
      #355/#357, все `ai-prosto`), edge-check evidence и одобряющее ревью на
      каждом из пяти candidate, **ноль** платных ревью моделью на
      candidate/finalize (везде prose-only scope-аттестация), ноль правок узлов
      после одобрения. Все пять рёбер — `PASS`; 24 круга edge-check, 26 находок,
      ложных ноль.
      **Прогон 2 из 2 выполнен 2026-09-23**, предмет — devtools#171; run-id
      `i5-traceless-noop-after-abandon-20260923-3e0ea3`, статус `completed`,
      evidence — `docs/evidence/2026-09-23-waves-live-run-2.md`. Четыре
      критерия §6.3 выполнены и здесь (6 актов: candidate #363/#365/#367/
      #369/#371 — все `merged_by=andrei-shtanakov`; конверты #364/#366/#368/
      #370/#372 — все `ai-prosto`; пять рёбер `PASS`; ноль платных ревью на
      candidate/finalize; ноль правок узлов после одобрения).
      **Различие двух прогонов — и есть смысл второго.** Оба выполнили
      §6.3, но прогон 1 сделал это ТОЛЬКО с обходом: отказ `405` на мерже
      finalize приходил 5 раз из 5, и на каждой волне оператор запускал
      внешний скрипт, ждавший зелени и повторявший `resume`. Прогон 2 шёл на
      дереве с правкой #362 и завершил каждую волну САМ: `405` — 0 из 5,
      ручных `resume` из-за незелёных проверок — 0, скрипт не запускался,
      пять конвертов смержены агентом подряд. Для флипа дефолта достаточно
      именно второго: сертифицируется контур без няньки, а не контур,
      доходящий до конца при участии оператора.
      Кругов edge-check: 24 (прогон 1) против 18 (прогон 2).
      **Условие S13 выполнено. Остаток пункта — РЕШЕНИЕ владельца** о
      переключении дефолта на `waves`; удаление прежнего пути S13 разводит
      с флипом («прежний путь удаляется отдельным пунктом») и здесь не
      смешивается.
      Наблюдения о механике, поднятые прогоном (подробности в evidence):
      devtools#277 воспроизведён 5/5 — обвязка не ждёт зелёных проверок перед
      мержем finalize; текст паузы после такого отказа называет уже влитый
      candidate вместо finalize-PR; `edge-findings.txt` переживает состояние,
      которое описывал (класс закрыт для `gate-findings.txt`, для edge — нет);
      доставка tasks-спеки идёт только от `spec-loop`, прогон, доведённый до
      `completed` раннером, остаётся без tasks-PR.
      Предусловие учёток (S7): автор candidate/finalize ≠ `ai-prosto`,
      иначе форджа не примет одобряющее ревью edge-check на свой же PR.

- [x] Обвязка §I12 ждёт обязательные проверки перед мержем finalize; `human-merge.sh` без `--admin` под защитой ветки @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:i12-finalize-merge-vs-branch-protection
      Принято 2026-09-23 из inbox devtools#277 (`from: spec-runner#480`) под
      тем же слагом. Триаж по живому волновому прогону
      `stray-md-disables-i8-for-legacy-v1-20260922-51772c` — исходная заявка
      подтверждена **частично**, и разница существенна:
      **Половина 1 опровергнута в текущем виде.** Заявка утверждала, что
      агентский мерж finalize недостижим по построению (6 отказов из 6 у
      spec-runner): автор PR — единственная учётка, чей approve имеет вес, а
      свой PR апрувить нельзя. В волновом контуре замкнутого круга больше
      нет: `--approve-node` публикует scope-аттестацию prose-only от
      `ai-prosto`, одобряющее ревью появляется без вызова модели, и **все
      пять** finalize-PR (#349/#351/#353/#355/#357) смержены агентом
      (`merged_by=ai-prosto`). Шаг аттестации доставлен ПОСЛЕ подачи #277.
      **Половина 1 сужена до настоящего остатка.** Обвязка зовёт мерж сразу
      после создания finalize-PR, когда `test` ещё `in progress`, и получает
      `405 Required status check "test" is in progress`. Отказ корректный и
      fail-closed (код 4, ничего не засчитано авансом), но происходит
      **ровно один раз на волну** — 5 из 5 в прогоне, все с одной причиной.
      Каждый стоит лишнего `resume`. Критерий закрытия: обвязка дожидается
      обязательных проверок (или ретраит с откатом) до попытки мержа, и
      живой прогон проходит без отказов этого класса; регрессия — finalize
      на незелёной голове не даёт ни мержа, ни 405 в журнале оператора.
      **ЗАКРЫТО 2026-09-23 целиком.** Половина 1 — кодом (#362,
      `_await_required_checks`); живая проверка: прогон 2 приёмки дал
      `405` — 0 из 5 против 5 из 5 в прогоне 1, ручных `resume` — 0,
      внешний скрипт не запускался, пять конвертов смержены агентом подряд
      (evidence `docs/evidence/2026-09-23-waves-live-run-2.md`).
      Половина 2 — **решением владельца, а не кодом** (#376): `--admin`
      не добавлен. Довод, которого не было при подаче заявки: под волновым
      режимом дыры нет — candidate несёт одобряющее ревью edge-check, и за
      две живые приёмки флаг не понадобился ни разу; волны стали дефолтом
      (#375), значит дыра осталась в уходящем пути, и выдавать обвязке
      право обходить защиту ради него — плохой размен. Вместо флага
      граница названа в трёх местах, где давалось обещание: шапка и usage
      скрипта, help Makefile, строка таблицы CLAUDE.md (её у скрипта не
      было вовсе). Issue devtools#277 закрыта с этой причиной.
      Ниже — состояние на момент триажа, сохранено как история решения.

      **Половина 2 в силе и в прогоне не проверялась.** `human-merge.sh`
      по-прежнему знает только `--merge|--squash`, флага `--admin` нет
      (сверено по файлу на `master`). В этом прогоне он не звался: candidate
      мержил владелец через UI, и защита ветки была удовлетворена, потому что
      **волновой candidate несёт одобряющее ревью edge-check** от `ai-prosto`.
      Значит дыра остаётся там, где у candidate одобряющего ревью нет —
      легаси-контур §I12 spec-runner, как и было заявлено. Решение («дать
      `--admin` с fail-closed сверкой логина» либо «перестать называть
      `make human-merge` способом совершить акт там, где он не работает») —
      за владельцем, оно же закрывает расхождение канона и практики.
      Влияние на второй живой прогон (`@id:waves-live-acceptance`): половина
      1 детерминирована и повторится 5 раз; без правки второй прогон измерил
      бы контур, которому на каждой волне нужна нянька оператора, — а
      сертифицировать флипом дефолта предполагается контур без неё.
      **Половина 1 закрыта PR этой ветки** (решение владельца 2026-09-23 —
      чинить ДО прогона 2): `_await_required_checks` в `approve_node` перед
      `ops.merge` опрашивает `pr_facts` (40 × 30 с), мержит только на
      зелёных, называет красную проверку поимённо и отказывает по потолку;
      каждый опрос сверяет `headRefOid` с пином конверта — ожидание вводит
      окно в минуты, и без пина в него поместился бы посторонний push.
      Предикат зелени — общий `governance/pr_checks.py` с `accept_pr`, не
      второй вычислитель. Защита ветки не тронута: ожидание ПЕРЕД мержем,
      не обход. **Остаток пункта — половина 2**: `--admin` в
      `human-merge.sh` в эту правку по решению владельца не включён.

- [x] Диагностика волнового контура врёт в двух местах: пауза после отказа мержа finalize называет уже влитый candidate, `edge-findings.txt` переживает состояние, которое описывал @owner:github:andrei-shtanakov @epic:eco.dark-factory @id:waves-diagnostics-name-wrong-state
      Оба наблюдены живьём в прогоне
      `stray-md-disables-i8-for-legacy-v1-20260922-51772c`, оба — про
      диагностику, а не про поведение, и потому чинятся раньше поведения.
      1. После отказа мержа finalize кнопка печатает
      `wave=N/5: ждём человеческий мерж candidate-PR #<candidate>`, хотя
      candidate уже влит человеком, а ждёт своих проверок finalize-PR. Текст
      отправляет человека делать сделанное.
      2. `edge-findings.txt` показывал находки прошлого круга после гейтовой
      остановки и после отказавшего запуска — читается как свежий. Класс тот
      же, что уже закрыт для `gate-findings.txt`
      (`@id:gate-findings-stale-on-green`): артефакт переживает состояние,
      которое описывал.
      **Закрыто PR этой ветки.** Диагноз оказался точнее: шаг edge СНИМАЕТ
      свой файл на входе — этого хватало, пока следующий круг до него
      доходит. Не доходит, когда заход встаёт раньше (на гейте): файл
      прошлого круга лежит рядом с чужим стопом и читается как его
      описание. Поэтому файлы находок снимаются там же, где `resume`
      сбрасывает op'ы (`_drop_findings_of`): сброшенный шаг и его находки
      уходят вместе, каким бы ни был следующий стоп. Номер PR в паузе:
      условие смотрело на ИСТИННОСТЬ `review_exit`, и нулевой код —
      аттестация прошла, мерж отказал — читался как «finalize ещё нет»;
      теперь заведённый finalize-PR сам по себе означает, что candidate
      влит, и пауза называет finalize.
...
- [ ] R16 runner выпускается из dev-scratch в devtools и работает на VPS отдельным сервисом (inbox devtools#382, slug: r16-runner-graduation; from: prograph-vault R16, vault#151–#166) @owner:github:andrei-shtanakov @id:r16-runner-graduation @epic:eco.tooling
      ...
      **Спека:** `docs/superpowers/specs/2026-09-25-r16-runner-graduation-design.md`
      (дизайн согласован с владельцем 2026-09-25: тот же VPS, свой
...
- [ ] selfcheck S1: самодиагностика devtools — статические пробы, граф использования, дубли, LLM-вызовы, дельта; отчёт-only (спека `docs/superpowers/specs/2026-09-25-selfcheck-design.md`, план `docs/superpowers/plans/2026-09-26-selfcheck-s1.md`; пара — PR #402) @owner:github:andrei-shtanakov @id:selfcheck-s1 @epic:eco.tooling
- [ ] selfcheck S2: `--fleet` — рёбра из всех репо манифеста, `confirmed` dead становится достижим @owner:github:andrei-shtanakov @id:selfcheck-s2 @blocked_by:todo://devtools/selfcheck-s1 @epic:eco.tooling
...
- [ ] selfcheck S4: отдельная спека — пробы, исполняющие код цели (clippy, credo, `mix xref`, knip): песочница, материализация зависимостей, проектная канарейка @owner:github:andrei-shtanakov @id:selfcheck-s4-spec @blocked_by:todo://devtools/selfcheck-s1 @epic:eco.tooling
