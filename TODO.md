...
- [ ] Новые модули «Предмета» DT доезжают в чек-лист обязательством, а не теряются между предметом поставки и наблюдаемым поведением @owner:github:andrei-shtanakov @id:task-bridge-subject-deliverables-unpinned @epic:eco.dark-factory
      ...
      **Порядок поставки фиксирован** (решение владельца 2026-09-21):
      ...
      Каждый барьер снят ВМЕСТЕ с тем, что делает его ненужным, а не
      раньше: барьер моста — срезом 2 вместе с переносом; переходный
      дефолт `RunState.allow_legacy_dt: True` — срезом 3 вместе с
      авторингом, выпускающим `dt_contract_version: 2`.
      **Срез 3 доставлен:** промпт авторинга несёт версию и грамматику
      ...
      файл постарше. «Любое
      сочетание безопасно» было неверно: без барьера мост принял бы бандл с
      объявленным `delivers` и молча потерял его при рендере по старому
      контракту.
      **Срез 1 доставлен (PR этой ветки):** `dt_contract_findings` —
