...
- [ ] Вызывающий контур исполняет stop rule ревью механически, а не прозой: targeted recheck только после блокирующей находки, non-gate находки круга не открывают, адресный режим `--base <последняя отревьюированная head>` @owner:github:andrei-shtanakov @id:review-loop-limit-enforcement @epic:eco.tooling
      ...
      **Осталось:** адресный recheck (`--targeted`) — вырезан из #259 решением
      владельца 2026-09-18 после трёх кругов ревью: stop rule стабилизировался
      за один круг, а адресный режим дал блокер в каждом (пустой диапазон по
      подставленному маркеру, пустой по несдвинувшейся голове, no-op фикс
      отпечатка, вердикт не раскрывает сужение диапазона). Отдельным PR;
      тестировать обязательно на fp-стенде — на стенде без отпечатка половина
      путей не наблюдается. Заявка — devtools#260.
