...
- [ ] Новые модули «Предмета» DT доезжают в чек-лист обязательством, а не теряются между предметом поставки и наблюдаемым поведением @owner:github:andrei-shtanakov @id:task-bridge-subject-deliverables-unpinned @epic:eco.dark-factory @trigger:"после утверждения контракта результатов поставки DT в devtools"
      ...
      **Поправка 2026-09-21: владелец установлен, и это мы.** Прежняя
      ...
      Открытые вопросы до утверждения: формулировка проверяемого
      обязательства (вида и идентификатора мало), адресная ссылка результата
      на пункт источника (`traces_to` связывает документы, не результаты) и
      версионирование контракта (отсутствие поля НЕ доказывает легаси).
