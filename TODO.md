...
- [ ] Ловить опору на удаляемую сущность: пункт не должен строиться на том, что удаляет другой пункт графа @owner:github:andrei-shtanakov @id:plan-check-deleted-dependency @epic:eco.plan-fields
      ...
      Важное уточнение, полученное ревью PR #132 на этом же пункте: удаляющий
      пункт путь НЕ называет — он говорит «переходник claude-review» голым
      именем. То есть критерий «сопоставлять пути» не поймал бы собственный
      мотивирующий случай, и это первое, что обязана учесть реализация: якорем
      служит путь, названный в теле ОДНОГО пункта, а во втором ищется путь ИЛИ
      его basename. Basename шумнее, поэтому пара «опирается» × «удаляет»
      остаётся обязательной, а порог решает замер.
