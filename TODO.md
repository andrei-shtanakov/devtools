...
- [ ] Харнесс-слой воркеров: один резолвер с обеими песочницами и structured output @owner:github:andrei-shtanakov @id:worker-harness-layer
      ...
      (1) Адаптер — `scripts/harness/claude`, обобщение `claude-review` на обе
      песочницы: `--sandbox read-only` → нынешние `--restricted --tools Read
      Glob Grep`; `--sandbox workspace-write` → write-флаги из `_author_argv`,
      но со схемой и тем же разбором конверта. Почему шим, а не python-модуль:
      ...
      модуль уровня репо, импортируемый `ops.py` и обоими воркерами; ключи
      `WORKER_HARNESS`/`WORKER_MODEL`, порядок как у `review-pr.sh`: флаг > env
      сессии > `~/.config/ai-prosto/harness.env` > вшитый codex. `review-pr.sh`
      оставляет свой shell-разбор того же файла — это осознанные два парсера
      одного формата, и их согласие закрепляется тестом (формат объявлен в
      самом harness.env: `KEY=VALUE`, без кавычек, файл парсится, не
      исполняется).
      ...
      битый конверт → код 3; приоритет env над файлом; согласие двух парсеров
      harness.env.
