...
на уровне вокабуляра. Owner-side копии тоже нет: референс-валидатор
`check-catalog-fixtures.py` читает этот же файл.
...
- **ATP** (набл. 2026-08-18, `atp-platform@6509727` — merge PR #295 по
  ...
  `kind` — `CatalogWarning` против `KNOWN_HARNESS_KINDS` (рукописная копия
  словаря до vocabulary.toml). Известных дивергенций нет.
